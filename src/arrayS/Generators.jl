...
#=▶=# function add(_addc, _c2i_constr, a...)
    p = _cis(_addc, a...)
    _c2i_constr(p, a...)
    _i2s(p, a...)
    p
end
_i2s(p, m, t, s, GD) = JuMP.@constraint(m, [t=t, s=s],
...
function add_pp(m, t, s, p, p0)
    pp = JuMP.@variable(m, [t=t, s=s])
    JuMP.@constraint(m, [t=t, s=s], pp[t, s] == p[t, s] + p0[t, s])
    pp
end
