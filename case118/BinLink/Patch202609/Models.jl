...
function multi_θ_mst!(genv, S, EVLmax, EEmax)
    ...
    (; m, o, refd, refi, S, N, Xl, Θ, Bv, Σt, CanSpawn, ch, Vv, rng, proceed, Ci, Cd)
end
...
function MIP_sub!(
    ...
    # inner Bin variables
    m[:bES] = bES = Dict((e,t,u) => bES_scalar(m, e, t, u, bES0) for e=eachindex(EEmax) for t=0:T for u=(0,1)) # here t really starts from 0
    m[:bx] = bx = Dict((a,t) => bx_scalar(m, a, t, bx0) for a=eachindex(EVLmax) for t=-1:T)
...
#         0 <= bu[a=eachindex(EVLmax), t=0:T] <= 1
#         0 <= bv[a=eachindex(EVLmax), t=0:T] <= 1
#         0 <= pDR[a=eachindex(EVLmax), t=0:T, u=(0,1)] <= 0 # DR of EVs (u=0: EV reduce their demand, u=1: EV increase their demand)
#         0 <= eV[a=eachindex(EVEmax), t=-1:T] <= (1-max(t,0)/10T)EVEmax[a] # storage UB (time variant)
#         pEV[a=eachindex(EVLmax), t=0:T]
#     end)
#     for (a,Pl)=enumerate(EVLmax) # Station inner
...
#     ES_2_sys(Enode, Ratea, F, T, gpe, pfe, pES)
#     EV_2_sys(EVnode, Ratea, F, T, gpe, pfe, pEV)
#     powermat_to_sys(WPmax, WmuMat, Wnode, T, gpe, pfe, Ratea, F)
#     powermat_to_sys(GPmax, GPref, Gnode, T, gpe, pfe, Ratea, F)
#     for (l,node)=enumerate(Lnode) # load - to system
#         LdMax, L01Curve = LPmax[l], LmuTup[Ltype[l]]
#         for t=0:T
#             Pl = LdMax * L01Curve[1+t]
...
#     end
#     JuMP.@constraint(m, [t=0:T], gpe[t] == 0)
#     JuMP.@variable(m, 0 <= pfA[b=eachindex(Ratea)])
#     for (b,Lim)=enumerate(Ratea), t=0:T
#         JuMP.@constraint(m, pfA[b] >=  pfe[b,t])
#         JuMP.@constraint(m, pfA[b] >= -pfe[b,t])
#     end
#     JuMP.@objective(m, Min, sum(pfA))
#     m, pfA
# end
