module Ben
import ..Models, ..Settings, Gurobi, JuMP
...
function get_ub(s::Int, Xl::Vector{Float64}, sub) # ub of Q_s(x)
    ...
    (ter == JuMP.OPTIMAL || ter == JuMP.TIME_LIMIT) || (@error(string("get_ub> ter = ", ter)); error())
...
_9(s, Xl, sub, ch) = (get_ub(s, Xl, sub); put!(ch, s))
